...
int main(void)
{
    ...
    int hits = 0;
    // 回答に表示する値を入れる文字型変数を定義します
    char ans_4, ans_3, ans_2, ans_1;

    if (ascii[0]==a) { //もし4桁目が一致していたらヒット+1
        hits += 1;
        ans_4 = a; // 4桁目が当たっていたら正解の数字を表示するので値を格納
        } else {
        ans_4 = '?';  // 正解ではないものは「？」が表示されます
        }

    if (ascii[1]==b) { //もし3桁目が一致していたらヒット+1
        hits += 1;
        ans_3 = b; // 3桁目が当たっていたら正解の数字を表示するので値を格納
        } else {
        ans_3 = '?'; // 正解ではないものは「？」が表示されます
        }

    if (ascii[2]==c) { //もし2桁目が一致していたらヒット+1
        hits += 1;
        ans_2 = c; // 2桁目が当たっていたら正解の数字を表示するので値を格納
        } else {
        ans_2 = '?'; // 正解ではないものは「？」が表示されます
        }

    if (ascii[3]==d) { //もし1桁目が一致していたらヒット+1
        hits += 1;
        ans_1 = d; // 1桁目が当たっていたら正解の数字を表示するので値を格納
        } else {
        ans_1 = '?'; // 正解ではないものは「？」が表示されます
        }
